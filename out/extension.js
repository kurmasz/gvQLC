...
function activate(context) {
    ...
    let askQuestionCommand = vscode.commands.registerCommand('extension.askQuestion', () => __awaiter(this, void 0, void 0, function* () {
        ...
        const panel = vscode.window.createWebviewPanel('askQuestion', 'Ask Question', vscode.ViewColumn.One, { enableScripts: true });
        panel.webview.html = `
  <!DOCTYPE html>
  <html lang="en">
  <head>
  ...
  `;
    ...
    let answerQuestionCommand = vscode.commands.registerCommand('extension.answerQuestion', () => __awaiter(this, void 0, void 0, function* () {
        ...
        const panel = vscode.window.createWebviewPanel('answerQuestion', 'Answer Question', vscode.ViewColumn.One, { enableScripts: true });
        panel.webview.html = `
              <!DOCTYPE html>
              <html lang="en">
              <head>
        ...
        panel.webview.onDidReceiveMessage((message) => __awaiter(this, void 0, void 0, function* () {
            if (message.answer) {
                ...
                try {
                    ...
                    const globalQuestionsPath = path.join(baseDir, 'questionsData.json');
                    const globalQuestions = loadQuestionData(globalQuestionsPath);
                    const globalIndex = globalQuestions.findIndex(q => q.filePath === question.filePath &&
                    ...
                    if (studentDir) {
                        ...
                        if (fs.existsSync(studentQuestionsPath)) {
                            const studentQuestions = loadQuestionData(studentQuestionsPath);
                            const studentIndex = studentQuestions.findIndex(q => q.filePath === question.filePath &&
                                q.range.start.line === question.range.start.line &&
                                q.timestamp === question.timestamp);
                            if (studentIndex !== -1) {
    ...
    let viewQuestionsAndAnswersCommand = vscode.commands.registerCommand('extension.viewQuestionsAndAnswers', () => __awaiter(this, void 0, void 0, function* () {
        ...
        const uniqueQuestions = allQuestions.filter((question, index, self) => index === self.findIndex(q => q.filePath === question.filePath &&
            q.range.start.line === question.range.start.line &&
            q.timestamp === question.timestamp));
    ...
    let addPersonalizedQuestionCommand = vscode.commands.registerCommand('extension.addPersonalizedQuestion', () => __awaiter(this, void 0, void 0, function* () {
        ...
        const loadExistingAnswers = () => __awaiter(this, void 0, void 0, function* () {
            try {
        ...
        const panel = vscode.window.createWebviewPanel('addPersonalizedQuestion', 'Add Quiz Question', vscode.ViewColumn.One, { enableScripts: true });
        // HTML content for the Webview
        panel.webview.html = `
<!DOCTYPE html>
<html lang="en">
...
<body>
    ...
    `;
        ...
        panel.webview.onDidReceiveMessage((message) => __awaiter(this, void 0, void 0, function* () {
            if (message.type === 'submitQuestion') {
                ...
                yield saveDataToFile('personalizedQuestions.json', personalizedQuestionsData);
                // Save answer to quiz_questions_answers.json if provided
                if (message.answer && message.answer.trim() !== '') {
                    try {
                        let answersData = yield loadExistingAnswers();
                        answersData.push({
                        ...
                        yield saveDataToFile('quiz_questions_answers.json', answersData);
    ...
    function saveDataToFile(filename, data) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield vscode.workspace.fs.writeFile(uri, Buffer.from(JSON.stringify(data, null, 2)));
        });
    }
    function openEditQuestionPanel(index) {
        ...
        const panel = vscode.window.createWebviewPanel('editQuestion', 'Edit Question', vscode.ViewColumn.One, { enableScripts: true });
        // HTML content for the Webview
        panel.webview.html = `
    <!DOCTYPE html>
    <html lang="en">
  ...
  `;
    ...
    //     const questionsWithAbsolutePaths = personalizedQuestionsData.map(question => {
    //       return {
    //         ...question,
    ...
    //           }
    //         }
    //         // If config file found, try to read quiz_directory_name
    //         if (configFileUri) {
    //           try {
    //             const fileData = await vscode.workspace.fs.readFile(configFileUri);
    //             const config = JSON.parse(fileData.toString());
    //             if (config.quiz_directory_name) {
    //               quizDirectoryName = config.quiz_directory_name;
    //             }
    //           } catch (error) {
    //             console.error("Error reading config file:", error);
    //           }
    //         }
    //         // Now look for students in the specified directory
    //         for (const folder of workspaceFolders) {
    //           const folderUri = folder.uri;
    ...
    //     questionsWithAbsolutePaths.forEach((question) => {
    ...
    let viewPersonalizedQuestionsCommand = vscode.commands.registerCommand('extension.viewPersonalizedQuestions', () => __awaiter(this, void 0, void 0, function* () {
        ...
        // Convert relative paths to absolute paths for display
        const questionsWithAbsolutePaths = personalizedQuestionsData.map(question => {
            return Object.assign(Object.assign({}, question), { filePath: path.join(workspaceRoot, question.filePath), relativePath: question.filePath });
        });
        if (!configData) {
            try {
                const configFilenames = ['cqlc.config.json', 'gvqlc.config.json'];
                ...
                if (configFileUri) {
                    const fileData = yield vscode.workspace.fs.readFile(configFileUri);
                    configData = JSON.parse(fileData.toString());
                    studentNameMapping = configData.studentNameMapping || {};
                }
                else {
                    vscode.window.showErrorMessage('No config file found. Press Command + Shift + P and select "Create Sample Config File".', { modal: true });
                    return;
                }
            }
            catch (error) {
        ...
        const getAllCISStudents = () => __awaiter(this, void 0, void 0, function* () {
            try {
                const cisStudents = new Set();
                let quizDirectoryName = "CIS"; // Default fallback
                // Get quiz directory name from config
                if (configData && configData.quiz_directory_name) {
                    quizDirectoryName = configData.quiz_directory_name;
                }
                // Look for students in the specified directory
                for (const folder of workspaceFolders) {
                    const folderUri = folder.uri;
                    if (folderUri.fsPath.includes(quizDirectoryName)) {
                        const files = yield vscode.workspace.fs.readDirectory(folderUri);
                        for (const [name, type] of files) {
                            if (type === vscode.FileType.Directory) {
                                cisStudents.add(name);
                            }
        ...
        const allCISStudents = yield getAllCISStudents();
        const questionsByStudent = {};
        for (const question of questionsWithAbsolutePaths) {
            const studentName = yield extractStudentName(question.filePath, configData);
            if (!questionsByStudent[studentName]) {
                questionsByStudent[studentName] = [];
            }
            questionsByStudent[studentName].push(question);
      ...
      `;
        ...
        const questionsTable = reorderedQuestions.map((question, index) => {
            const studentName = extractStudentName(question.filePath, configData);
            const labelColor = getLabelColor(studentName);
            const filePathParts = question.relativePath.split('/');
            let shortenedFilePath = filePathParts.length > 2
                ? `.../${filePathParts.slice(-3).join('/')}`
      ...
      `;
        ...
        // Create a Webview Panel for viewing personalized questions
        const panel = vscode.window.createWebviewPanel('viewPersonalizedQuestions', 'View Quiz Questions', vscode.ViewColumn.One, { enableScripts: true });
        panel.webview.html = `
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    ...
    let generatePersonalizedQuizCommand = vscode.commands.registerCommand('extension.generatePersonalizedQuiz', () => __awaiter(this, void 0, void 0, function* () {
        ...
        // Automatically search for config files in the root folder
        ...
        const possibleConfigs = ['cqlc.config.json', 'gvqlc.config.json'];
        ...
        if (!configFilePath) {
            vscode.window.showErrorMessage('No config file (cqlc.config.json or gvqlc.config.json) found in the root folder.\n\n' +
                'Press Command + Shift + P (or Ctrl + Shift + P on Windows/Linux), then select "Create Sample Config File" to generate one.', { modal: true });
            return;
        }
        // Load config file
        let config;
...
${question.text || 'No question text provided'}
    ...
    ${question.highlightedCode ? `<pl-code language="${config.language}">\n${question.highlightedCode}\n</pl-code>` : ''}
            ...
            const infoAssessmentContent = {
                ...
                allowAccess: [
                    Object.assign({ mode: "Public", uids: [studentName], credit: 100, timeLimitMin: config.timeLimitMin, startDate: config.startDate, endDate: config.endDate }, (config.password && { password: config.password })),
                    {
                        mode: "Public",
                        credit: 0,
...
`;
        ...
        fs.writeFileSync(path.join(instructorQuestionFolderPath, 'question.html'), combinedHTMLContent);
        ...
        fs.writeFileSync(path.join(instructorAssessmentPath, 'infoAssessment.json'), JSON.stringify(instructorInfoAssessmentContent, null, 2));
        vscode.window.showInformationMessage(`Successfully generated personalized quiz!\n\n` +
            ...
            `Instructor assessment: ${instructorAssessmentPath}`, { modal: true });
    }));
    let createConfigCommand = vscode.commands.registerCommand('extension.createConfig', () => __awaiter(this, void 0, void 0, function* () {
        const workspaceFolders = vscode.workspace.workspaceFolders;
        if (!workspaceFolders) {
            vscode.window.showErrorMessage('No workspace folder is open');
            return;
        ...
        if (fs.existsSync(configPath)) {
            const overwrite = yield vscode.window.showWarningMessage('Config file already exists. Overwrite?', { modal: true }, 'Yes', 'No');
            if (overwrite !== 'Yes')
                return;
        }
        const sampleConfig = {
    ...
    context.subscriptions.push(highlightAndCommentCommand, viewCommentsCommand, askQuestionCommand, answerQuestionCommand, viewQuestionsAndAnswersCommand, addPersonalizedQuestionCommand, viewPersonalizedQuestionsCommand, generatePersonalizedQuizCommand, createConfigCommand);
}
/**
