...
<body>
  ...
  let viewPersonalizedQuestionsCommand = vscode.commands.registerCommand('extension.viewPersonalizedQuestions', async () => {
    ...
    for (const question of questionsWithAbsolutePaths) {
      const studentName = await extractStudentName(question.filePath, configData);
      if (!questionsByStudent[studentName]) {
        questionsByStudent[studentName] = [];
      }
      questionsByStudent[studentName].push(question);
...
<body>
  ...
  let generatePersonalizedQuizCommand = vscode.commands.registerCommand(
    ...
    async () => {
      ...
      if (!configFilePath) {
        vscode.window.showErrorMessage(
          'No config file (cqlc.config.json or gvqlc.config.json) found in the root folder.\n\n' +
          'Press Command + Shift + P (or Ctrl + Shift + P on Windows/Linux), then select "Create Sample Config File" to generate one.',
          { modal: true }
        );
        return;
      }
